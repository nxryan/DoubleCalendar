...
(function (factory) {
    ...
    $.fn.dateRangePicker = function (opt) {
        ...
        opt = $.extend(true,
		{
		    ...
		    shortcuts:
			{
			    //'prev-days': [3,5,7], //过去几天
			    //'next-days': [3,5,7], //未来几天
			    //'current-days': [0],//今天日期（此参数只支持）

			    'prev': ['week', 'month', 'year'],//过去
			    'current': ['day','week', 'month', 'mon-up', 'mon-mid', 'mon-down'] //当前
			    //'next' : ['week','month'] //未来
			},
		    customShortcuts: [],
		    inline: false,
        ...
        function init_datepicker() {
            ...
            //快捷方式：点击事件 (by ryan 2018-1-24)
            //(备注：86400000==1天，86400000表示一天24小时的总毫秒数)
            ...
            box.find('[shortcut]').click(function () {
                ...
                if (shortcut.indexOf('day') != -1) {
                    ...
                    if (isNaN(day)) { //注：今天日期传入的参数是一个字符串‘day’
                    ...
                    var dir = 0;
                    var dirtype = "";
                    if (shortcut.indexOf('prev,') != -1) {
                        dir = -1;
                        dirtype = "prev"; //上一周
                    }
                    else if (shortcut.indexOf('next,') != -1) {
                        dir = 1;
                        dirtype = "next";//下一周
                    }
                    else if (shortcut.indexOf('current,') != -1) {
                        dir = -1;
                        dirtype = "current";//本周
                    }
                    ...
                    var stopDay;
                    if (dirtype == "prev") {
                        stopDay = opt.startOfWeek == 'monday' ? 1 : 6;
                    }
                    else if (dirtype == "next") {
                        stopDay = opt.startOfWeek == 'monday' ? 0 : 0;
                    }
                    else {
                        stopDay = opt.startOfWeek == 'monday' ? 1 : 6;
                    }
                    if (dirtype == "current") {
                        end = new Date(end.getTime() + (stopDay - end.getDay()+1) * 86400000);
                        start = new Date(end.getTime() - stopDay * 86400000);
                    }
                    else {
                        end = new Date(end.getTime());
                    ...
                    else if (shortcut.indexOf('current,') != -1) { //curent当前月
                        dir = -1;
                        dirtype = "current";
                    }
                    if (dirtype == "prev") {
                ...
                else if (shortcut.indexOf("mon-up") != -1) {
                    start = currentMonth(end);
                    start.setDate(1);
                    end.setDate(10);
                }
                else if (shortcut.indexOf("mon-mid") != -1) {
                    start = currentMonth(end);
                    start.setDate(11);
                    end.setDate(20);
                }
                else if (shortcut.indexOf("mon-down") != -1) {
                    start = currentMonth(end);
                    start.setDate(21);
                    //end = nextMonth(start);
                ...
                else if (shortcut.indexOf('current-year') != -1) {
                    var dir = shortcut.indexOf('prev,') != -1 ? -1 : 1;
                    start = new Date();
                    start.setFullYear(end.getFullYear() + dir);
                    start.setMonth(0);
        ...
        function createDom() {
            ...
            if (opt.showShortcuts) {
                ...
                if (data) {
                    ...
                    //今天日期
                    //if (data['current-days'] && data['current-days'].length > 0) {
                    //    for (var i = 0; i < data['current-days'].length; i++) {
                    //        var name = data['current-days'][i];
                    //        if (name == 0) {
                    //            name = (data['current-days'][i] > 1) ? lang('current-day') : lang('current-day');
                    //        }
                    //        else {
                    //            name += (data['current-days'][i] > 1) ? lang('current-day') : lang('current-day');
                    //        }
                    ...
                    //            name = (data['current-days'][i] > 1) ? lang('current-day') : lang('current-day');
                    //        }
                    //        html += ' <a href="javascript:;" shortcut="day,' + data['current-days'][i] + '">' + name + '</a>';
                    //    }
                    //    html += '</span>';
                    //}
                    ...
                    if (data['current'] && data['current'].length > 0) {
                        for (var i = 0; i < data['current'].length; i++) {
                            var name = lang('current-' + data['current'][i]);
                            html += ' <a href="javascript:;" shortcut="current,' + data['current'][i] + '">' + name + '</a>';
                        }
                        html += '</span>';
                    }
                }
